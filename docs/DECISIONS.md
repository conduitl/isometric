...
| Figurine animation / movement / world interaction | Post-v1 phases, in that order (the Aug 2026 asset-pipeline direction) | v1 ships the static pipeline: a character authored as a WORLD (the Pip fixtures), `figurineFromWorld` conversion, and a self-contained `figurine` entity component drawn scaled into one cell in all three projections. The component is opaque data to the format, so animation frames (a figurine per pose) and a mover system can arrive additively; the per-entity embedded payload trades file size for portability and can migrate to a world-level asset table behind one formatVersion bump |
